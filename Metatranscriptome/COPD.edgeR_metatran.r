...
deseq <- function(metadata,counts,recounts,sampletype,comparison) {
        ...
        #idx <- rowSums(relcounts3 >= 0.004 ) >= 9 #Metatranscriptome
        #df <- relcounts3[idx,]
        idx <- rowSums(relcounts3 >= 0.004 ) >= 9 #Metatranscriptome
        df <- relcounts3[idx,]
        ...
        heatmap3<-df[,wanted2]
        write.table(heatmap_raw3,file="Metatranscriptome_BAL_raw_pruned_count_table.txt", sep="\t", col.names = NA, row.names = TRUE)
        write.table(heatmap3,file="Metatranscriptome_BAL_Rel_Abundance_pruned_count_table.txt", sep="\t", col.names = NA, row.names = TRUE)
        ...
        #Create Heatmap
        pdf("Metatranscriptome_pruned_BAL_Heatmap.pdf", height = 15, width = 20)
        ...
        library(cowplot)
        pdf("Metatranscriptome.BAL_PCOA_Loadings.pdf", height = 5, width = 10)
